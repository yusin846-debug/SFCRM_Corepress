...
export default class CpPortalLanding extends LightningElement {
  ...
  renderedCallback() {
    if (this.pendingInquiryProduct && this.isCatalogModalOpen) {
      const select = this.template.querySelector(
        '.catalog-modal [name="product"]'
      );
      if (select) {
        const match = this.equipmentProducts.find(
          (p) => p.name === this.pendingInquiryProduct
        );
  ...
  collectInquiry() {
    const value = (name) =>
      this.template
        .querySelector(`.catalog-modal [name="${name}"]`)
        ?.value?.trim() || "";
    const product = value("product");
    const matchedProduct = this.equipmentProducts.find(
    ...
    return {
      ...
      consent: Boolean(
        this.template.querySelector('.catalog-modal [name="consent"]')?.checked
      ),
      marketingConsent: Boolean(
        this.template.querySelector('.catalog-modal [name="marketingConsent"]')
          ?.checked
      )
    };
  }
