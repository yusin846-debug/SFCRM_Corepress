...
const PART_RULES = [
  ...
  { match: (n) => /Gearbox|Gear/i.test(n) && /Filter/i.test(n), src: partOilFilter },
  ...
  { match: (n) => /After-Filter|Pre-Filter|Filter Element/i.test(n), src: partAirFilter },
  ...
  { match: (n) => /Pressure Gauge|압력\s*게이지/i.test(n),   src: partPressureGauge },
  { match: (n) => /Diagnostic|진단 키트|진단키트/i.test(n),  src: partDiagKit },
  { match: (n) => /HMI|모니터링/i.test(n),                   src: partHMI },
  { match: (n) => /토탈솔루션|Control Panel|컨트롤 패널/i.test(n), src: partControlPanel },
